...
- ✅ **Keychain 会话提示**：触发系统弹窗前向用户明示，需显式 consent
...
│   ├── fetch_cookie.py   # 三级零弹窗 cookie 提取（peanuts→CDP→Keychain）
...
- 检测并安装 Python 依赖（pycryptodome、rookiepy，已装则跳过）
- 检测并创建 uv 3.10 环境（小红书签名，已就绪则跳过）
- 检测并安装 playwright chromium（抖音 fallback，已就绪则跳过）

## Cookie 提取（三级零弹窗 + Keychain 会话提示）

`fetch_cookie.py` 使用三级解密策略，**优先避开系统弹窗**：

| 级别 | 方案 | 弹窗 | 说明 |
|------|------|------|------|
| 1 | peanuts 纯数据解密 | 无 | 从 Cookies SQLite 用固定密钥 AES 解密，仅旧版/无保护 profile 有效 |
| 2 | CDP 拿明文 | 无 | 连接已运行浏览器（`--remote-debugging-port`），返回明文 |
| 3 | Keychain 兜底 | **需 consent** | 用 rookiepy 读 Keychain 解密，需用户显式同意 |

**策略链**：`peanuts,cdp,keychain`（config.yaml 的 `decrypt_strategy` 可配置）

**Keychain 兜底会话提示**（不自动弹窗）：
- peanuts/CDP 失败后，keychain 默认跳过，仅打印引导
- 触发 Keychain 弹窗前，agent 必须用通用「向用户询问」明示：
  - 将访问浏览器 'Safe Storage' 条目，触发一次系统弹窗
  - 点 "Always Allow" 后不再弹窗，授权仅绑定当前解释器
- 用户同意后运行：`python3 scripts/fetch_cookie.py <platform> --keychain-consent`
- 用户拒绝 → 跳过该平台 / 用 justoneapi / 仅 CDP
...
诊断脚本会检测：
- **依赖**：pycryptodome、rookiepy、uv、xhshow、playwright、chromium、依赖 skills
...
```bash
# 1. 提取 cookie（需用户同意，三级策略自动降级）
python3 scripts/fetch_cookie.py xiaohongshu
#    peanuts/CDP 失败时，如需 Keychain 兜底：
python3 scripts/fetch_cookie.py xiaohongshu --keychain-consent
...
- **零弹窗优先**：peanuts→CDP 两级零弹窗，Keychain 兜底需显式 consent，绝不自动弹窗
