...
| 2 | CDP 拿明文 | 无 | 连接已运行浏览器（`--remote-debugging-port`），`Network.getAllCookies` 返回明文 |
| 3 | Keychain 兜底 | 仅一次 | 用 rookiepy 读 Keychain 解密（解密正确，已授权不弹窗） |

**策略链**：`peanuts,cdp,keychain`（config.yaml 的 `decrypt_strategy` 可配置）
- 前级失败自动降级到下一级
- Keychain 仅作为最后兜底，且只在已授权时静默工作
- 全部失败 → 打印明确提示（不自动弹窗），引导用户配置 justoneapi 或手动授权
...
- **零弹窗优先**：peanuts→CDP 两级零弹窗，Keychain 仅作兜底且只在已授权时静默
