...
- **清理**：任务结束自动移入 `~/.Trash`，绝不上传、不打印、不进日志、不写入 opencode.json
- **同意门禁**：每次触发 cookie 提取前，主 agent 必须用 `question` tool 向用户明示并获同意；用户可随时选择"改用已有 skill / 跳过该平台"

## ⚠️ 重要限制：粉丝量查询必须主 Agent 直接执行

**严禁使用子 agent（subagent）调用粉丝量查询 skills**。

原因：子 agent 调用 skill 时会触发用户确认提示，导致调研不完整或流程中断。

正确方式：在主 agent（当前对话）中直接调用 Bash 工具执行 Python 脚本查询粉丝量。

```bash
# 在主 agent 中直接执行，不通过子 agent
...
- 子 agent 调研出问题无法中途沟通，造成 token 浪费
...
# 密钥路径可配置：环境变量优先，默认 opencode secrets 目录
KEY_OK=0
...
> ⚠️ 每次触发 cookie 提取前，必须用 `question` tool 向用户明示"将读取 X 浏览器 Y 平台 cookie，用途 Z"，并提供"改用已有 skill / 跳过该平台"降级选项。用户未同意则跳过。
...
- **首次运行**需询问用户确认，并检测旧目录（如 `/Users/11169285/Documents/调研/output/`）：
  - 沿用旧目录，还是使用新目录？
...
每开始一位人物前，先创建todo：
```
todowrite: 人物=XXX, status=in_progress
```

完成后标记：
```
todowrite: 人物=XXX, status=completed
```
