...
# 3. 检测并安装 Python 依赖（pycryptodome，AES 解密）
log "🔍 检测 pycryptodome..."
if /usr/bin/python3 -c "from Crypto.Cipher import AES" 2>/dev/null; then
  log "✅ pycryptodome 已安装，跳过"
else
  log "🔧 安装 pycryptodome..."
  pip3 install --user pycryptodome 2>/dev/null || warn "pycryptodome 安装失败，请手动执行: pip3 install --user pycryptodome"
fi

# 3.1 检测并安装 rookiepy（Keychain 兜底）
log "🔍 检测 rookiepy..."
