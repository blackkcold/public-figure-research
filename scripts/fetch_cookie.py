...
def extract(platform, strategy="peanuts,cdp,keychain", keychain_consent=False):
    """按策略链提取 cookie（peanuts→CDP→Keychain，带超时防循环）。

    keychain_consent=False 时，keychain 兜底不执行（不触发系统弹窗）。
    需用户显式同意后传 True 才走 keychain 兜底。
    """
    ...
    if "cdp" in strategies:
        ...
        if found:
            return found, browser
    if "keychain" in strategies and keychain_consent:
...
def main():
    ...
    parser.add_argument("--keychain-consent", action="store_true", help="显式同意后允许 Keychain 兜底（触发一次系统弹窗）")
    ...
    cookies, browser = extract(args.platform, args.strategy, keychain_consent=args.keychain_consent)

    if not cookies:
        print(f"未提取到 {args.platform} cookie")
        if "keychain" in args.strategy and not args.keychain_consent:
            print("提示（不自动弹窗）：")
            print("  1. peanuts/CDP 均无法获取。如需 Keychain 兜底（触发一次系统弹窗），")
            print("     运行: python3 scripts/fetch_cookie.py <platform> --keychain-consent")
            print("  2. 或配置 justoneapi key")
            print("  3. 或手动在 Keychain Access 允许 'Safe Storage' 条目")
        else:
            print("提示（不自动弹窗）：")
